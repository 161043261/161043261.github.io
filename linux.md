...
sudo apt install apt-transport-https build-essential ca-certificates cmake curl gdb git golang-go net-tools ninja-build openssh-server pkg-config tree vim wget zsh
...
export JAVA_OPTS="-Dfile.encoding=UTF-8 -Dsun.stdout.encoding=UTF-8 -Dsun.stderr.encoding=UTF-8"
...
```shell
# client
cat ~/.ssh/id_rsa.pub | ssh tiancheng.hang@221.226.84.186 -p 18022 "cat >> ~/.ssh/authorized_keys" && ssh tiancheng.hang@221.226.84.186 -p 18022
...
| left-cmd \| right-cmd   | 将 left-cmd 的输出作为 right-cmd 的输入  |
| ----------------------- | ---------------------------------------- |
| left-cmd && right-cmd   | 只有 left-cmd 执行成功，才执行 right-cmd |
| left-cmd \|\| right-cmd | 只有 left-cmd 执行失败，才执行 right-cmd |
...
# 创建 RXE 网卡
sudo rdma link add crxe type rxe netdev ens33 # client
sudo rdma link add srxe type rxe netdev ens33 # server
# 删除 RXE 网卡
sudo rdma link delete crxe
sudo rdma link delete srxe
# 打印 RXE 网卡列表
rdma link
# 查询 RDMA 设备信息
ibv_devinfo -d roce
# 测试 send 操作的带宽
ib_send_bw -d srxe           # server
ib_send_bw -d crxe client.ip # client
```
