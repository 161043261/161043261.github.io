...
- 属性选择器
  ...
  - `.selector[attr="val"]` 选择有 attr 属性, 并且 attr="val" 的元素, 例如 `a[href="https://example.com"]` 选择有 href 属性, 且 href="https://example.com" 的 a 元素
  - `.selector[attr^="val"]` 选择有 attr 属性, 并且 attr 以 val 开头的元素, 例如 `div[class^="bg-"]` 选择存在 class 属性, 且 class 以 bg- 开头的 div 元素
  - `.selector[attr$="val"]` 选择有 attr 属性, 并且 attr 以 val 结尾的元素, 例如 `div[class$="-green"]` 选择存在 class 属性, 且 class 以 -green 结尾的 div 元素
  - `.selector[attr*~*="val"]` 选择有 attr 属性, 并且 attr 包含 val 的元素, 例如 `div[class*="-"]` 选择存在 class 属性, 且 class 包含 - 的 div 元素
- 伪类选择器
  - `:hover` 鼠标悬浮时选择元素
  - `:active` 鼠标按下时选择元素
...
- justify-items 单元格内容的水平位置
  - `justify-items: start` 单元格左对齐
  - `justify-items: end` 单元格右对齐
  - `justify-items: center` 单元格水平居中
  - `justify-items: stretch` 拉伸以填充单元格宽度 (默认)
- align-items: 单元格内容的垂直位置
  - `align-items: start` 单元格上对齐
  - `align-items: end` 单元格下对齐
  - `align-items: center` 单元格垂直居中
  - `align-items: stretch` 拉伸以填充单元格高度 (默认)
