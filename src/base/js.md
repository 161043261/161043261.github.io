...
- **JS 引擎线程 (主线程)**: 一个页面 (一个渲染进程) 中只有一个 JS 引擎线程, 负责将同步任务压入同步任务栈 (函数调用栈), 执行所有同步任务, 异步任务 (包括宏任务和微任务)
- **事件触发线程**: 负责将异步任务加入队列, 宏任务加入宏任务队列, 微任务加入微任务队列
- **定时器触发线程**: 执行 setTimeout, setInterval 的线程
- **网络线程**: 执行 XMLHttpRequest, fetch, postMessage 的线程
- **I/O 线程**: 负责文件 I/O, IPC 进程间通信等

单线程本质: JS 引擎线程 (主线程) 负责执行所有同步任务, 异步任务 (包括宏任务和微任务); 宏任务触发可能依赖其他线程
...
- 异步任务: 包括宏任务和微任务
  - 宏任务
    - `<script>` 整体代码, 即同步任务栈 (函数调用栈) 中所有的同步任务
...
所有同步任务 -> 所有同步任务的微任务队列 -> 宏任务 1 -> 宏任务 1 的微任务队列 -> 宏任务 2 -> 宏任务 2 的微任务队列 -> ...

1. 执行同步任务栈 (函数调用栈) 中所有的同步任务, 将同步任务的的所有微任务加入微任务队列
2. 清空微任务队列: 按序执行所有微任务, 如果微任务执行过程中产生新的微任务, 则一并执行
3. 从宏任务队列中取出并执行 1 个宏任务, 将该宏任务的所有微任务加入微任务队列
4. 重复 2,3

如果将整体代码, 即同步任务栈 (函数调用栈) 中所有的同步任务也视为一个宏任务, 则执行顺序简化为: 每一个事件循环, 先执行 1 个宏任务, 再执行该宏任务的所有微任务, 最后进入下一个事件循环
