...
class ChatPage extends Component<void, IState> {
  state = {
    inputVal: "",
  };
...
- RSC, React Server Components 服务器组件
  - 服务器组件不会被打包, 减小 bundle 体积
  - 局部水合, 避免全量水合导致的性能损耗
  - 支持流式加载 `Transfer-Encoding: chunked`, 减少 FCP 首次内容绘制时间
